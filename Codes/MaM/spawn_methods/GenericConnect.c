...
void MAM_Init_job_connect_port() {
  if(mall->port_name != NULL) return;

  if(MAM_ROOT == mall->myId) {
  mall->port_name = (char *)malloc(MPI_MAX_PORT_NAME * sizeof(char));
  MPI_Open_port(MPI_INFO_NULL, mall->port_name);
  } else {
    mall->port_name = (char *)malloc(1 * sizeof(char));
    mall->port_name[0] = '\0';
  }
}
...
void MAM_Prepare_job_comms(int is_children) {
  ...
  /*if(mall->service_name == NULL) { 
    mall->service_name = malloc(MPI_MAX_PORT_NAME * sizeof *mall->service_name);
    if(mall->myId == MAM_ROOT && !is_children) { snprintf(mall->service_name, 5, "NULL"); } 
    else { mall->service_name[0] = '\0'; }
  }*/
...
int MAM_Connect_jobs_as_source(void) {
  ...
  // Try to detect if targets are ready
  MPI_Comm_accept(mall->port_name, MPI_INFO_NULL, MAM_ROOT, aux_comm, &aux_inter_comm);
  ...
  return MAM_I_JOB_CONNECTED;
}

/*
void MAM_Connect_jobs_as_target(void) {
  int open_p;
  Spawn_ports *spawn_port = malloc(sizeof *spawn_port);
  MPI_Comm aux_inter_comm = MPI_COMM_NULL;

  open_p = mall->myId == MAM_ROOT ? 1 : 0;
  ...
  mall->service_name = NULL; // In free ports service name is released for root, but not set to NULL
}
*/
...
void MAM_Connect_jobs_as_target(void) {
  ...
  MPI_Info_set(info, "timeout", "0");
  MPI_Comm_connect(mall->port_name, info, MAM_ROOT, mall->comm, &aux_inter_comm);
...
void MAM_Connect_jobs_as_children(void) {
  ...
  // Join jobs into a single intercommunicator
  MPI_Comm_accept(mall->port_name, MPI_INFO_NULL, MAM_ROOT, mall->intercomm, &aux_inter_comm);
