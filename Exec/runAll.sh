...
# Parameter 3(Optional): Wether expands should consider RMS. Must be 0(False) or 1(True). False by default
# Parameter 4(Optional) - Maximum amount of time in seconds needed by a single execution. Default value is 0, which indicates infinite time. Must be a positive integer.
...
if [ $# -ge 4 ] #Max time per execution in seconds
then
  limit_time=$(($4 * $qty / 60 + 1))
fi
